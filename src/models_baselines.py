...
if __name__ == "__main__":
    ...
    for well_idx, wellname in enumerate(wells):
        ...
        z = 3000.0 - np.cumsum(np.abs(rng.normal(0.32, 0.02, size=rows_per_well)))
        frames.append(
            pd.DataFrame(
                {
                    "WELLNAME": wellname,
                    "MD": md,
                    "X": 1000.0 + well_idx * 35.0 + np.cumsum(rng.normal(1.0, 0.1, size=rows_per_well)),
                    ...
                    "TVT": 15.0 + well_idx + md * 0.33 + rng.normal(0.0, 0.08, size=rows_per_well),
    ...
    assert set(model.oof_predictions_.keys()) == set(BaselineEnsembleModel.BACKEND_ORDER)
    assert len(model.oof_predictions_rf) == len(mock_df)
    assert len(model.oof_predictions_et) == len(mock_df)
    assert len(model.oof_predictions_hist) == len(mock_df)
    ...
    print("models_baselines.py smoke test passed.")
